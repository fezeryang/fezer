...
import MobileReadingNav from "@/components/blog/MobileReadingNav";
import { useStaggerReveal } from "@/hooks/use-stagger-reveal";
// Submodule imports (not the @/content/loaders barrel): the barrel
// re-exports renderBlogMarkdown, which would pull marked + sanitize-html
// back into the detail-page client graph.
import { getPostBySlug } from "@/content/loaders/posts";
import { getRenderedPost } from "@/content/loaders/renderedPosts";
...
export default function BlogPostDetail({ slug }: BlogPostDetailProps) {
  ...
  const headerShown = useStaggerReveal();
      ...
      <div className="relative min-h-screen overflow-x-hidden bg-[#f2f0ed] text-[#3e3c3a]">
        <DampedScrollView>
          <main className="mx-auto flex min-h-screen max-w-3xl flex-col items-center justify-center px-6 text-center">
            <div className={`t-stagger ${headerShown ? "is-shown" : ""}`}>
              <p className="t-stagger-line t-stagger-line--1 text-xs font-mono tracking-[0.24em] text-[#8e8a85] uppercase">
                Blog / 404
              </p>
              <h1 className="t-stagger-line t-stagger-line--2 mt-4 text-3xl font-semibold text-[#2a2a2a]">
                未找到该文章
              </h1>
              <p className="t-stagger-line t-stagger-line--3 mt-3 text-sm text-[#6a6560]">
                该链接可能已失效，或文章尚未发布。
              </p>
            </div>
            <div className="mt-8 flex items-center gap-3">
              <Link
                href="/blog/surface"
                className="rounded-full border border-[#d1cdc7] bg-[#f9f8f6] px-4 py-2 text-xs font-mono uppercase tracking-[0.14em] text-[#3e3c3a] transition-colors duration-[250ms] hover:bg-[#ece8e2]"
              >
                返回展示页
              </Link>
              <Link
  ...
  const { html: rendered, sections } = useMemo(() => {
    const result = getRenderedPost(post.slug) ?? { html: "", sections: [] };
    return {
      html: result.html,
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-[#f7f5f0] text-[#3e3c3a]">
            ...
            <div className="mx-auto max-w-4xl">
              ...
              <div className={`t-stagger ${headerShown ? "is-shown" : ""}`}>
                <header className="mb-16 text-center">
                  <p className="t-stagger-line t-stagger-line--1 mb-8 text-[11px] font-mono uppercase tracking-[0.25em] text-[#a19d96]">
                  ...
                  <h1 className="t-stagger-line t-stagger-line--2 font-songti text-4xl font-normal leading-[1.35] tracking-wide text-[#1c1b1a] md:text-5xl lg:text-[46px]">
                    {post.title}
                  </h1>
                  {post.excerpt && (
                    <p className="t-stagger-line t-stagger-line--3 mx-auto mt-8 max-w-2xl font-songti text-lg italic leading-[2] text-[#75706b]">
                      {post.excerpt}
                    </p>
                  )}
                </header>

                <div className="t-stagger-line t-stagger-line--4 mx-auto h-px w-12 bg-[#1c1b1a]/30" />
              </div>
            </div>
          </section>
      ...
      <MobileReadingNav sections={sections} />
      <Navigation />
      <GrainOverlay />
      <CustomCursor />
    </div>
