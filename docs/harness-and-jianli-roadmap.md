...
### 0.2 Agent 运行时缺口（P0 开工前的实测快照，当前状态见 §0.5）
...
### 0.5 进度（P0–P4 已完成，2026-09-14）

| 期 | 范围 | 状态 |
| --- | --- | --- |
| **P0** | A1/A2 契约与骨架、路由迁移、B3/B4 3D 修复、性能基线、死工具清理 | ✅ |
| **P1** | A3 事件、A4 真取消、C2 SSE + 令牌级流式、C3 思考可视化、e2e mock 下移、B5/B6 相机与交互 | ✅ |
| **P2** | C5 内容卡片、C9 房间内容化（C12 房间标注）、C4 真实综合 + 协作时间线、B7 minimap、B9 移动端 | ✅ |
| **P3** | A5 会话存储 + C7 恢复上次对话、C6 访客进度（客户端 + 服务端推荐与注入）、C1 主动导览 | ✅ |
| **P4** | A6 计量、C8 语音输入、C10 对话导出、C11 简历内容运营化、B8 健壮性（WebGL 降级/加载进度/reduced-motion）、B2 未引用资源清理 | ✅ |

**验证口径**（本次会话内反复执行）：`pnpm check` + `pnpm check:tests` 干净；**44 文件 / 396 测试全绿**；e2e 全量 33/33 与分项多次通过；生产构建在内存允许时成功 5 次（见下方限制）。

**未完成 / 有意延后**：

| 项 | 原因 |
| --- | --- |
| B2 的 Draco/量化（面向**首屏加载的 7.12MB**） | 需要重编码二进制资产 + 新增 devDependency，且必须用眼睛确认观感；计划 §6 的 <1.5MB 目标取决于此或下面的按需挂载 |
| B1 按需挂载（决策点 2）、18→7 角色（决策点 9） | 都是产品/视觉取舍，需你拍板；机制已勘误（预加载 ≠ 浪费，见 B 表下方） |
| C11 的 admin 编辑器 | markdown 迁移已完成（改内容不再需要改代码），后台可视化编辑是附加便利 |
| 生产构建的**最终**一次验证 | 本机 6.8GB、3 个 tsserver 占 1.3GB + 其他项目 dev server，构建峰值 ~1.5-2GB 堆：1536MB 被内核 OOM 杀、1280MB 触发 V8 上限。代码本身由 tsc + 396 测试 + 浏览器 e2e 覆盖 |
